...
For live evidence, each planned checkpoint must preserve the actual fixture or commit revision, runtime, operation, terminal `blob_id`, and cold-recall observation. I will not count an accepted asynchronous job, a timeout, or a local digest as storage proof. The practical result is a memory agent that learns from mistakes without treating similarity as permission to repeat them elsewhere.
